...
export default function PropertyDetail() {
  ...
  const handleConfirmPayment = async () => {
    ...
    if (!isLoaded) {
      toast.error('Payment gateway failed to load. Please check your connection.', { id: loadingToast });
      return;
    }
    ...
    const options = {
      ...
      handler: async function (response: any) {
        ...
        try {
          ...
          toast.success('Payment successful! Booking confirmed.', { id: 'payment-verification' });
          setActionMode(null);
          setReservationResult(null);
        } catch (e: any) {
          toast.error(e.message || 'Payment verification failed', { id: 'payment-verification' });
        }
      },
      prefill: {
  ...
  const handleScheduleVisit = async () => {
    ...
    try {
      await requestVisit.mutateAsync({
        ...
        scheduled_at: `${visitForm.date}T${visitForm.time === '10:00 AM' ? '10:00:00' : visitForm.time === '12:00 PM' ? '12:00:00' : visitForm.time === '2:00 PM' ? '14:00:00' : visitForm.time === '4:00 PM' ? '16:00:00' : '18:00:00'}`,
        visit_type: 'physical',
      });
  ...
  const handleVirtualTour = async () => {
    if (!tourForm.name || !tourForm.phone || !tourForm.slot) {
      toast.error('Please fill in all required fields.');
      return;
    }
    try {
      await requestVisit.mutateAsync({
        ...
        scheduled_at: new Date().toISOString(), // Default to now for tour request
        visit_type: 'virtual',
        notes: `Preferred Slot: ${tourForm.slot}`,
      });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Chat Widget */}
      <PropertyChat propertyName={property.name} propertyId={property.id} isOpen={chatOpen} onClose={() => setChatOpen(false)} />
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {!reservationResult ? (
            ...
            <div className="space-y-4">
              ...
              <DialogFooter>
                <Button onClick={handleConfirmPayment} disabled={confirmReservation.isPending} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground gap-2">
                  <CreditCard size={16} /> {confirmReservation.isPending ? 'Processing...' : 'Pay with UPI / Cards'}
                </Button>
              </DialogFooter>
            </div>
          )}
      ...
      <Dialog open={actionMode === 'schedule_visit'} onOpenChange={(o) => !o && setActionMode(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <Button className="w-full bg-accent hover:bg-accent/90 text-accent-foreground" onClick={handleScheduleVisit} disabled={requestVisit.isPending}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div><Label>Preferred Slot</Label>
              <Select value={tourForm.slot} onValueChange={v => setTourForm(f => ({ ...f, slot: v }))}>
            ...
            <Button className="w-full bg-accent hover:bg-accent/90 text-accent-foreground" onClick={handleVirtualTour} disabled={requestVisit.isPending}>
              {requestVisit.isPending ? 'Booking...' : 'Book Virtual Tour'}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
