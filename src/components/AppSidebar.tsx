...
import {
  ...
  Map, Sparkles, Receipt, Globe, LogOut
} from 'lucide-react';
          ...
          <div className="mt-2 mx-0.5 p-2.5 rounded-lg" style={{ background: 'hsl(var(--sidebar-hover))' }}>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-accent/20 flex items-center justify-center">
                <span className="text-[9px] font-bold text-accent">{(user?.email?.[0] || 'U').toUpperCase()}</span>
              </div>
              <div className="min-w-0">
                <p className="text-[11px] font-medium truncate capitalize" style={{ color: 'hsl(var(--sidebar-active-fg))' }}>{role || 'User'}</p>
                <p className="text-[9px] truncate" style={{ color: 'hsl(var(--sidebar-fg))' }}>{user?.email}</p>
              </div>
            </div>
          </div>
        </div>
