...
import { useSendMessage } from '@/hooks/usePublicData';
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="px-3 py-2.5 border-t border-border bg-background">
            {showIdentForm || !userInfo ? (
              <form onSubmit={handleIdentify} className="space-y-2">
                <p className="text-[10px] text-muted-foreground px-1">Please identify yourself to chat with an agent</p>
                <div className="flex gap-2">
                  <Input name="name" placeholder="Name" required className="h-8 text-xs" />
                  <Input name="phone" placeholder="Phone" required className="h-8 text-xs" />
                </div>
                <Button type="submit" size="sm" className="w-full h-8 text-xs bg-accent text-accent-foreground">Start Chatting</Button>
              </form>
            ) : (
              <div className="flex gap-2">
                <Input
                ...
                />
                <Button size="sm" className="h-9 w-9 p-0 bg-accent hover:bg-accent/90" onClick={() => sendMessage(input)} disabled={sendMessageHook.isPending}>
                  {sendMessageHook.isPending ? <Loader2 size={14} className="animate-spin text-accent-foreground" /> : <Send size={14} className="text-accent-foreground" />}
                </Button>
              </div>
            )}
          </div>
